...
router.all('/get-full', function (req, res, next) {
    util.JSONChecker(res, req.body, async (data) => {
        const result = await mconnects.query("SELECT ex.eid, ex.etitle, ex.ebanner, ex.estart_date, ex.eend_date, e.*, u.* FROM `rs_epurchases` e LEFT JOIN `rs_users` u ON e.buid=u.uid LEFT JOIN `rs_events` ex ON e.beid=ex.eid", {
            plain: false,
            raw: true,
...
router.all('/get-event-full', function (req, res, next) {
    util.JSONChecker(res, req.body, async (data) => {
        //return error
        if (data.beid) {
            const result = await mconnects.query("SELECT ex.eid, ex.etitle, ex.ebanner, ex.estart_date, ex.eend_date, e.*, u.* FROM `rs_epurchases` e LEFT JOIN `rs_users` u ON e.buid=u.uid LEFT JOIN `rs_events` ex ON e.beid=ex.eid WHERE e.beid=" + data.beid + " GROUP BY e.bid", {
                plain: false,
                raw: true,
            });
...
router.all('/get-id-full', function (req, res, next) {
    util.JSONChecker(res, req.body, async (data) => {
        //return error
        if (data.buid) {
            const result = await mconnects.query("SELECT ex.eid, ex.etitle, ex.ebanner, ex.estart_date, ex.eend_date, e.*, u.* FROM `rs_epurchases` e LEFT JOIN `rs_users` u ON e.buid=u.uid LEFT JOIN `rs_events` ex ON e.beid=ex.eid WHERE e.buid=" + data.buid, {
                plain: false,
                raw: true,
            });
            if (result[0].length > 0) {
